...
- **Email Studio**: Prepare a transactional email for a human to edit, review, and approve in an MCP Apps-capable client. HTTP drafts are private to their MCP session; local stdio uses a private local broker only to bridge the host's MCP App process. Drafts expire after 15 minutes, send once on approval, and accept Base64 attachments only. An HTTP session or local stdio broker holds up to 40 MiB of decoded attachment data, and each email must stay within Resend's 40 MB Base64-encoded attachment limit. Clients without MCP Apps support receive a review-only preview; use `send-email` when direct delivery is intentional.
