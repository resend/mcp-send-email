/**
 * Webhook event payload types mirroring resend-node, with `message_id` included on
 * all email-related webhook `data` objects per the Resend API.
 *
 * @see https://github.com/resend/resend-node/blob/main/src/webhooks/interfaces/webhook-event.interface.ts
 */
...
export interface BaseEmailEventData {
  broadcast_id?: string;
  created_at: string;
  email_id: string;
  message_id: string;
...
export interface ReceivedEmailEventData {
  email_id: string;
  created_at: string;
  from: string;
  to: string[];
